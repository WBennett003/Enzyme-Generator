    ...
    def step_row(self, row):
        ...
        if len(self.columns) == len(row): # if there is an an error with the row
            ...
            if int(length) < self.max_aa_seq: # if the sequence is longer than the standard length
                ...
                if min(tokens) >= 0.0:
                    reaction = np.zeros(self.reaction_size) #self.reaction_encoder.encode(smilerxn, self.reaction_size)
        ...
        with open(self.inp_file_dir, 'r') as f:
            ...
            for i in range(n):
                if i % step == 0:
                    new_time = time.time()
                    perc = i / n 
                    diff = last_time - new_time
                    ETA = new_time + (n-i) * diff
                    print(f" {i} / {n} | {round(perc, 3)} | ETA {time.ctime(ETA)}")
                    last_time = new_time
